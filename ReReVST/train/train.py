...
parser.add_argument('--log', type=int, default=1000, help='number of iterations to save checkpoints and figures')
...
def LoadPretrained(checkpoint):
    # Try loading the checkpoint into the style_net
    try:
        style_net.load_state_dict(torch.load(checkpoint[0], map_location=lambda storage, loc: storage))
    except:
        # If loading fails, try loading specific parts of the model
        try:
            ...
            # If loading specific parts fails, handle it accordingly
            style_net.Decoder.conv_kernel = nn.Conv2d(512, 512, kernel_size=1)
            style_net.load_state_dict(torch.load(checkpoint[0], map_location=lambda storage, loc: storage))
            del style_net.Decoder.conv_kernel

# To train from scratch, omit the code related to checkpoint loading here
# Set states
style_net.train()
print(style_net)
...
use_mpi=False, use_video=False, pin_memory=True)
    ...
    for iteration, sequence in enumerate(loader):
        ...
        if iteration % opt.log == 0:
            ...
            if cur_total_loss < min_total_loss:
                ...
                torch.save(style_net.state_dict(), '%s/style_net-latest-epoch-%d(%.2e)(W_%d_%d)(Sigma_%.2e).pth' % 
                    (opt.outf, opt.epoches , opt.temporalWeight, opt.data_w * opt.data_motion_level, opt.data_w * opt.data_shift_level, opt.data_sigma * opt.data_noise_level))
