...
import torch
import torch.utils.data as data     # Python iterable over a dataset
from torch.autograd import Variable     # implementing automatic differentiation of arbitrary scalar valued functions
import torch.nn.functional as F        #tensor functions

from torchvision import datasets, transforms    # popular datasets, model architectures, and common image transformations
                                                # for computer vision
import torchvision.utils as vutils              # various utilities, mostly for vizualization
...
class MPIDataset(data.Dataset):
    ...
    def __init__(self, loadSize=288, fineSize=256, flip=True, 
                    mpi_path="C:/Users/Marios/Desktop/DIPLOMA/ReReVST/ReReVST-Code-master/data/MPI/training", style_path="../data/style/sample"):
        ...
        Folder = [os.path.normpath(path) for path in Folder]        #Normalize path for windows
        for folder in Folder:
            ...
            for i in range(2,ImgNumber+1):
                ...
                assert os.path.exists("%s/flow/%s/frame_%04d.flo"%(mpi_path,name_folder,i-1))
                self.flow_list.append("%s/flow/%s/frame_%04d.flo"%(mpi_path,name_folder,i-1))
    ...
    def __getitem__(self, index):
        ...
        backwardflow = read_flo_file(self.flow_list[index])
...
class FrameDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if first_frame is None:
            # If image is None, skip it and return a new sample from the dataset
...
def get_loader(batch_size, loadSize=288, fineSize=256, flip=True, 
                    content_path="./data/content/", style_path="./data/style/", num_workers=16, use_mpi=False, use_video=False, pin_memory=True):
                #
    ...
    loader = data.DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers, pin_memory=pin_memory)
    return loader
