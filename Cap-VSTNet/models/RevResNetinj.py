...
class RevResNet(nn.Module):
    ...
    def reshape_channels(self, input_tensor, new_num_channels):
        # Get the current shape of the input tensor
        current_shape = input_tensor.shape
        # Calculate the number of times to repeat the channels
        repeat_factor = new_num_channels // current_shape[1]
        # Repeat the channels along the channel dimension
        expanded_tensor = input_tensor.repeat(1, repeat_factor, 1, 1)
        # Slice to keep only the first 'new_num_channels' channels
        reshaped_tensor = expanded_tensor[:, :new_num_channels, :, :]
        return reshaped_tensor
    
    def inverse_reshape_channels(self, input_tensor, original_num_channels):
        # Get the current shape of the input tensor
        current_shape = input_tensor.shape
        # Calculate the number of times channels were repeated
        repeat_factor = current_shape[1] // original_num_channels
        # Slice to keep only the first 'original_num_channels' channels
        reshaped_tensor = input_tensor[:, :original_num_channels, :, :]
        return reshaped_tensor
    ...
    def _forward(self, x):
        ...
        x = split(x)
        x1 = self.reshape_channels(x[0], 16)
        x2 = self.reshape_channels(x[1], 16)
        x = (x1,x2)
    ...
    def _inverse(self, x):
        ...
        x1 = self.inverse_reshape_channels(x[0], 1)
        x2 = self.inverse_reshape_channels(x[1], 2)
        x = merge(x1,x2)
