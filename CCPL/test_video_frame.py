...
for i,(content,contentName) in enumerate(content_loader):
    ...
    # Save the result frame as .png with the same name as input
    result_frame = transfer.squeeze(0).cpu().numpy()
    result_frame = (result_frame * 255).astype(np.uint8)
    # Extract the file extension from contentName (assuming it's a valid filename)
    file_extension = contentName.split('.')[-1]
    if not file_extension:
        file_extension = 'png'  # Default to 'png' if no extension found
    result_frame_filename = contentName.split('.')[0] + '.png'  # Force '.png' extension
    result_frame_pil = Image.fromarray(result_frame.transpose(1, 2, 0))
    result_frame_pil.save(os.path.join(args.output, result_frame_filename))
